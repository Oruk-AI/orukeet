...
root=Path('/home/nathanroll/parakeet-ft');dest=Path('/dev/shm/orukeet-regression-ft-20260907/native-ft4035')
model=dest/'orukeet-v0.1.0rc1-q8.gguf';runtime=root/'inference_20260905/cuda';fixture=root/'gabor_half_20260906/fixtures/jfk.wav'
def sha(p):
 with p.open('rb') as f:return hashlib.file_digest(f,'sha256').hexdigest()
assert sha(model)=='9b0c77604c7679d6da68d0014e9f753f3d893c26115f078847316efb1083bd0f'
...
receipt=dict(status='passed',model='Orukeet FT-4035 Q8',model_sha256=sha(model),fixture_sha256=sha(fixture),python=sys.version.split()[0],binding_sha256=sha(Path(orukeet.nvidia.__file__)),runtime_library_sha256={p.name:sha(p) for p in sorted((runtime/'lib').iterdir()) if p.is_file() and '.so' in p.name},results=results,scope='Two repeated JFK transcriptions per device in the existing Linux CUDA container, using the same CUDA SDK for CPU selection. No timing comparison, standalone CPU package or cross-platform app claim.')
(dest/'linux-q8-native-smoke.json').write_text(json.dumps(receipt,indent=2)+'\n');print(json.dumps(receipt),flush=True)
