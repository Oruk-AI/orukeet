"""Finite, counted passes over selected data with unchanged fitted Gabor rows.
...
def main():
    ...
    a = p.parse_args()
    if a.resume or a.stop_after_step or a.smoke_steps:
        p.error('This counted-pass runner starts a fresh finite run; use a saved NeMo parent to continue.')
    ...
    epochs = plan.get('epochs', 1)
    assert epochs >= 1
    batches = []
    for epoch in range(epochs):
        batches.extend(make_batches(rows, seed + epoch,
                                    max_seconds=plan['batching']['max_padded_seconds'],
                                    max_utterances=plan['batching']['max_utterances']))
    planned_exposures = Counter(i for batch in batches for i in batch)
    assert len(planned_exposures) == len(rows)
    assert set(planned_exposures.values()) == {epochs}
    ...
    run = dict(plan_sha256=sha(a.plan), prepared_sha256=sha(a.prepared), parent_sha256=sha(a.parent),
               ...
               rows=sum(map(len, batches)), unique_rows=len(rows), data_passes=epochs, smoke=bool(a.smoke_steps),
    ...
    for batch_index, batch in enumerate(loader, next_batch):
        ...
        actual_exposures.update(group_rows)
    ...
    if not a.smoke_steps:
        ...
        assert dict(seen) == {k:v * epochs for k,v in expected.items()}
        assert actual_exposures == planned_exposures
    ...
    audit = dict(frozen_gabor_rows_unchanged=12288, batchnorm_statistics_unchanged=True,
                 ...
                 exactly_one_pass=(epochs == 1 and not bool(a.smoke_steps)), data_passes=epochs,
                 every_record_presentations=epochs, unique_training_rows=len(rows),
                 actual_exposure_counts_sha256=hashlib.sha256(json.dumps(
                     {rows[i]['uid']:actual_exposures[i] for i in sorted(actual_exposures)},
                     sort_keys=True, separators=(',', ':')).encode()).hexdigest(), optimizer_steps=step)
    del final_state
    if not a.smoke_steps:
